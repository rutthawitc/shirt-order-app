...
const [previewImage, setPreviewImage] = useState<string | null>(null);
  ...
  const handleCustomerInfoChange = (field: keyof CustomerInfo, value: string | File | null | boolean) => {
    ...
    try {
      ...
      const itemsWithDetails = orderItems.map(item => {
        ...
        return {
          ...
          subtotal: (design?.price ?? 0) * item.quantity
        };
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData?.message || 'เกิดข้อผิดพลาดในการส่งคำสั่งซื้อ');
      }
      ...
      <Card>
        ...
        <Alert variant="destructive" className="mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="mb-8">
              <h3 className="text-lg font-semibold mb-4">แบบเสื้อที่มีให้เลือก</h3>
                ...
                {SHIRT_DESIGNS.map(design => (
                  <Card key={design.id} className="overflow-hidden">
                    ...
                    <div className="p-4">
                      ...
                      <p className="font-semibold text-lg">{design.price} บาท</p>
                    </div>
                  </Card>
                ))}
              </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <Select
                          value={item.design}
                          onValueChange={(value) => handleItemChange(index, 'design', value)}
                        >
                          <SelectTrigger>
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(value) => handleItemChange(index, 'quantity', parseInt(value))}
                        >
                          <SelectTrigger>
                    ...
                    <div className="flex items-center space-x-4">
                      <img 
                        src={SHIRT_DESIGNS.find(d => d.id === item.design)?.images[0]}
                        alt="ตัวอย่างเสื้อที่เลือก"
                        className="w-20 h-20 object-cover rounded"
                      />
                      <div className="text-right flex-1">
                        <div className="text-sm text-gray-600">
                          {SHIRT_DESIGNS.find(d => d.id === item.design)?.description}
                        </div>
                        <div className="font-semibold">
                          ราคา: {(SHIRT_DESIGNS.find(d => d.id === item.design)?.price ?? 0) * item.quantity} บาท
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block mb-2">ที่อยู่จัดส่ง {!customerInfo.isPickup && <span className="text-sm text-gray-500">(ค่าจัดส่ง)</span>}</label>
                  <Textarea
                    value={customerInfo.address}
                    onChange={(e) => handleCustomerInfoChange('address', e.target.value)}
                    placeholder={customerInfo.isPickup ? "ไม่ต้องระบุที่อยู่ (รับหน้างาน)" : "กรอกที่อยู่จัดส่ง"}
                    disabled={customerInfo.isPickup}
                  />
                </div>

                <div>
                  <label className="block mb-2">อัพโหลดสลิปการโอนเงิน</label>
                  <div className="flex items-center space-x-2">
                    <Input
                      type="file"
                      accept="image/*"
      ...
      <div className="mt-2">
        ...
        />
            ...
            <div className="text-right space-y-2">
              <div className="text-sm text-gray-600">
                ราคาสินค้ารวม: {orderItems.reduce((total, item) => {
                  const design = SHIRT_DESIGNS.find(d => d.id === item.design);
                  return total + (design?.price ?? 0) * item.quantity;
                }, 0)} บาท
              </div>
